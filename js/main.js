...
// Display products and cart amount and clear button on page load
document.addEventListener("DOMContentLoaded", () => {
    addClearCartButton();
    updateCartAmount();
    document.querySelector("body > div.small").innerHTML = "<h2 class='title'>Featured Products</h2>";
    const row = document.createElement("div");
    row.classList.add("row");
    document.querySelector("body > div.small").append(row);
    let i = 0;
    for (i = 0; i < 4; i++) {
        row.append(createProductCard(products.data[i]));
    }
    document.querySelector("body > div.small").innerHTML += "<h2 class='title'>Latest Products</h2>";
    const rowLatest = document.createElement("div");
    rowLatest.classList.add("row");
    document.querySelector("body > div.small").append(rowLatest);
    for (i = 4; i < products.data.length; i++) {
        rowLatest.append(createProductCard(products.data[i]));
    }
